...
namespace AIMathProject.API.Controllers
{
    ...
    public class PaymentController : ControllerBase
    {
        ...
        public async Task<IActionResult> PaymentCallBackVnPay()
        {
            ...
            var orderId = response.OrderId != null ? $"<p><span class=\"label\">Mã đơn hàng:</span><span class=\"value\">{response.OrderId}</span></p>" : "";
            var transactionId = response.TransactionId != null ? $"<p><span class=\"label\">Mã giao dịch:</span><span class=\"value\">{response.TransactionId}</span></p>" : "";
            var vnp_Amount = query.ContainsKey("vnp_Amount")
                ...
                : "";
            var vnp_BankCode = query.ContainsKey("vnp_BankCode") ? $"<p><span class=\"label\">Ngân hàng:</span><span class=\"value\">{query["vnp_BankCode"]}</span></p>" : "";
            var vnp_BankTranNo = query.ContainsKey("vnp_BankTranNo") ? $"<p><span class=\"label\">Mã giao dịch ngân hàng:</span><span class=\"value\">{query["vnp_BankTranNo"]}</span></p>" : "";
            var vnp_CardType = query.ContainsKey("vnp_CardType") ? $"<p><span class=\"label\">Loại thẻ:</span><span class=\"value\">{query["vnp_CardType"]}</span></p>" : "";
            var vnp_OrderInfo = query.ContainsKey("vnp_OrderInfo") ? $"<p><span class=\"label\">Thông tin đơn hàng:</span><span class=\"value\">{HttpUtility.UrlDecode(query["vnp_OrderInfo"])}</span></p>" : "";
            var vnp_PayDate = query.ContainsKey("vnp_PayDate") ? $"<p><span class=\"label\">Thời gian thanh toán:</span><span class=\"value\">{query["vnp_PayDate"]}</span></p>" : "";
            var vnp_TransactionStatus = query.ContainsKey("vnp_TransactionStatus") ? $"<p><span class=\"label\">Trạng thái giao dịch:</span><span class=\"value\">{query["vnp_TransactionStatus"]}</span></p>" : "";
            var vnp_TxnRef = query.ContainsKey("vnp_TxnRef") ? $"<p><span class=\"label\">Mã tham chiếu:</span><span class=\"value\">{query["vnp_TxnRef"]}</span></p>" : "";
